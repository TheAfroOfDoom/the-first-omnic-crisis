# Dictate TITAN WALK FORWARD ROTATION-4-SPECIFIC FIRST STEP Module
...
execute @s[tag=titanMustNotStep,score_clock=0] ~ ~ ~ execute @e[type=armor_stand,tag=titanLegNextStep,r=10,c=1] ~ ~ ~ detect ~ ~-1 ~-2 air * scoreboard players tag @e[type=armor_stand,tag=titanIsWalkingForwardFirstStep,r=10,c=1] add titanMustStepDown
