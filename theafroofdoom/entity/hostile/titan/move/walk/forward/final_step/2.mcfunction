# Dictate TITAN WALK FORWARD ROTATION-2-SPECIFIC FINAL STEP Module
...
execute @s[tag=titanMustNotStep,score_clock=0] ~ ~ ~ execute @e[type=armor_stand,tag=titanLegNextStep,r=10,c=1] ~ ~ ~ detect ~-2 ~-1 ~ air * scoreboard players tag @e[type=armor_stand,tag=titanIsWalkingForward,r=10,c=1] add titanMustStepDown
